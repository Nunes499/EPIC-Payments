...
from app.services.environment_separation_service import (
    get_environment_separation_status,
)
...
def cloud_integrity(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Não foi possível verificar a integridade cloud.",
        ) from exc
...
def backup_recovery(
    ...
    del current_user
    result = get_backup_recovery_status()
    if result.get("status") == "error":
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=(
                result.get("error")
                or "Não foi possível verificar o estado dos backups."
            ),
        )
    return result


@router.get("/environment-separation")
def environment_separation(
    current_user: User = Depends(require_admin),
):
    del current_user
    result = get_environment_separation_status()
    if result.get("status") == "error":
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=(
