...
function StatusBadge({ status }: { status: string }) {
  ...
  if (["failed", "error", "deleted", "voided"].includes(normalized)) {
    return (
...
export default function PagamentosPage() {
  const [items, setItems] = useState<PaymentReferenceItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await getPayments(statusFilter, search);
      setItems(data);
    } catch (loadError) {
      setError(
  ...
  useEffect(() => {
    ...
    async function refreshAutomatically() {
      try {
        const result = await refreshPendingPayments();
        if (cancelled) return;

        if (result.updated > 0) {
          const data = await getPayments("all", "");
          if (cancelled) return;
          setItems(data);
        }
      } catch {
  ...
  const counts = useMemo(() => {
    ...
    for (const item of items) {
      if (item.display_status === "paid") {
        result.paid += 1;
      } else if (item.display_status === "expired") {
        result.expired += 1;
      } else {
        result.pending += 1;
      }
  ...
  const lastVisibleItem = Math.min(currentPage * PAGE_SIZE, items.length);
  ...
  async function handleRefreshOne(item: PaymentReferenceItem) {
    ...
    <AppLayout hideHeader>
      <main className={styles.page}>
        <header className={styles.header}>
          <div>
            <span className={styles.kicker}>EPIC PAYMENTS</span>
            <h1>Pagamentos</h1>
            <p>
          ...
          >
            ...
            {refreshing ? "A verificar..." : "Atualizar estados"}
          </button>
        </header>
        ...
        <section className={styles.tableCard}>
          {loading ? (
            ...
            <div className={styles.empty}>
              <CreditCard size={34} />
              <strong>Ainda não existem referências para mostrar.</strong>
              <span>
            ...
            <>
              ...
              <div
                style={{
                  ...
                  flexWrap: "wrap",
                  padding: "16px 18px",
                  borderTop: "1px solid rgba(76, 107, 132, 0.12)",
                ...
                >
                  <button
                    ...
                    style={{
                      ...
                      padding: "0 14px",
                      borderRadius: "9px",
                      border: "1px solid rgba(45, 105, 148, 0.20)",
                      background: currentPage === 1 ? "#f1f5f8" : "#ffffff",
                      color: currentPage === 1 ? "#a4b0ba" : "#1c5f8e",
                      fontSize: "11px",
                      fontWeight: 800,
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: "1px solid rgba(45, 105, 148, 0.20)",
                      background:
                        currentPage === totalPages ? "#f1f5f8" : "#ffffff",
                      color:
                        currentPage === totalPages ? "#a4b0ba" : "#1c5f8e",
                      fontSize: "11px",
                      fontWeight: 800,
                      cursor:
